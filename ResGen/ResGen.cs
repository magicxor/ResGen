...
namespace ResGen
{
    public static class ResGen
    {
        ...
        public static void Main(string[] args)
        {
            ...
            else
            {
                ...
                for (; index1 < stringList.Count && ResGen.errors == 0; ++index1)
                {
                    ...
                    {
                        string str4 = stringList[index1].StartsWith("/D:", StringComparison.OrdinalIgnoreCase) || stringList[index1].StartsWith("-D:", StringComparison.OrdinalIgnoreCase) || stringList[index1].StartsWith("/d:", StringComparison.OrdinalIgnoreCase) || stringList[index1].StartsWith("-d:", StringComparison.OrdinalIgnoreCase) ? stringList[index1].Substring(3) : stringList[index1].Substring(8);
                        char[] chArray = new char[1]{ ',' };
                        foreach (string str5 in str4.Split(chArray))
                        {
                            if (str5.Length == 0 || str5.Contains("&") || str5.Contains("|") || str5.Contains("("))
                                ResGen.Error(SR1.GetString("InvalidIfdef", (object) str5));
                            ResGen.definesList.Add(str5);
                        }
                    }
                    ...
                    else if (stringList[index1].Equals("/AllowUntrustedFiles", StringComparison.OrdinalIgnoreCase) || stringList[index1].Equals("-AllowUntrustedFiles", StringComparison.OrdinalIgnoreCase))
        ...
        {
            Console.WriteLine(SR1.GetString("UsageOnWindows", (object) "4.8.9037.0", (object) CommonResStrings.CopyrightForCmdLine));
        ...
        private class ResGenRunner
        {
            ...
            {
                this.bufferedOutput.Add((Action) (() => Console.Error.WriteLine(formatString, args)));
            }
            ...
            {
                this.bufferedOutput.Add((Action) (() => Console.WriteLine(formatString, args)));
            }

            private void BufferWrite(string formatString, params object[] args)
            {
                this.bufferedOutput.Add((Action) (() => Console.Write(formatString, args)));
            }
                ...
                out string sourceFile)
            {
                ...
                {
                    this.BufferWriteLine();
                    foreach (string str6 in unmatchable)
                        this.Error(SR1.GetString("UnmappableResource", (object) str6), inputFileName);
                }
                else
            ...
            private bool ReadResources(string filename, bool useSourcePath)
            {
                ...
                if (format == ResGen.Format.Assembly)
                {
                    ...
                    {
                        this.Error(SR1.GetString("MOTW", (object) filename));
                        return false;
                    }
                ...
                else
                {
                    ...
                    switch (format)
                    {
                        ...
                        case ResGen.Format.XML:
                            ...
                            ResXResourceReader reader = ResGen.assemblyList == null ? new ResXResourceReader(filename) : new ResXResourceReader(filename, ResGen.assemblyList.ToArray());
                            if (useSourcePath)
                            {
                                string fullPath = Path.GetFullPath(filename);
                                reader.BasePath = Path.GetDirectoryName(fullPath);
                            }
                            this.ReadResources(readerInfo, (IResourceReader) reader, filename);
                            break;
            ...
            {
                Stack<string> searchForAll = new Stack<string>();
                bool flag1 = false;
                using (ResGen.LineNumberStreamReader numberStreamReader = new ResGen.LineNumberStreamReader(fileName, (Encoding) new UTF8Encoding(true), true))
                {
                                ...
                                if (str3.StartsWith("ifdef ", StringComparison.InvariantCulture) || str3.StartsWith("ifndef ", StringComparison.InvariantCulture) || str3.StartsWith("if ", StringComparison.InvariantCulture) || str3.StartsWith("If ", StringComparison.InvariantCulture))
                                {
                                    ...
                                    if (str1.Length != 0 && !str1.Contains("&") && !str1.Contains("|") && !str1.Contains("("))
                                    {
                                        if (str3.StartsWith("ifndef", StringComparison.InvariantCulture))
                                            str1 = "!" + str1;
                                        searchForAll.Push(str1);
                                        flag1 = !ResGen.IfdefsAreActive((IEnumerable<string>) searchForAll, (IList<string>) ResGen.definesList);
                                    }
                                    else
                                        goto label_14;
                                }
                    ...
                    label_68:
                    if (searchForAll.Count > 0)
                        throw new ResGen.TextFileException(SR1.GetString("UnbalancedIfdefs", (object) searchForAll.Pop()), fileName, numberStreamReader.LineNumber - 1, 1);
                }
            }
                ...
                using (StreamWriter streamWriter = new StreamWriter(fileName, false, Encoding.UTF8))
                {
                    foreach (ResGen.Entry resource in reader.resources)
                    {
                        ...
                        else
                        {
                            ...
                            streamWriter.WriteLine("{0}={1}", (object) name, (object) str2);
                        }
                    }
                }
            }
